...
with col_center:
    ...
    st.markdown("### ☄️ Globaler Markt-Scanner")
    st.markdown(
...
def get_german_tickers():
    ...
    mdax_50 = [
        ...
        "TMV.DE", "TKA.DE", "8TRA.DE", "TUI1.DE", "UTDI.DE", "WCH.DE", "RHM.DE",
        "S92.DE",
    ]
...
# Hauptlogik im zentrierten Bereich platzieren
with col_center:
    if st.button("☄️ Scan starten", type="primary", use_container_width=True):
        ...
        df_crashes = pd.concat(
            [df_c_stocks, df_c_comm, df_c_curr, df_c_crypto, df_c_bond], ignore_index=True
        )
        df_spikes = pd.concat(
            [df_s_stocks, df_s_comm, df_s_curr, df_s_crypto, df_s_bond], ignore_index=True
        )
        ...
        if not df_crashes.empty:
            ...
            st.dataframe(
                df_crashes.style.format({"Wochenänderung (%)": "{:.2f}%"}).background_gradient(
                    subset=["Wochenänderung (%)"], cmap="Reds_r"
                ),
        ...
        if not df_spikes.empty:
            ...
            st.dataframe(
                df_spikes.style.format({"Wochenänderung (%)": "{:.2f}%"}).background_gradient(
                    subset=["Wochenänderung (%)"], cmap="Greens"
                ),
                use_container_width=True,
            )
        else:
